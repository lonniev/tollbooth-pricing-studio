...
///    flow. `AuthorityDetailView.neonApiKeyCourierParams` is that seam.
///
...
final class PersistenceStatusCourierTests: XCTestCase {
    ...
    /// The courier request targets exactly `NEON_API_KEY` and carries the
    /// Authority's identity, endpoint, and credential service through to the
    /// shared courier sheet — landing the user in the identical flow.
    func testNeonApiKeyCourierParamsTargetsNeonApiKey() throws {
        let url = try XCTUnwrap(URL(string: "https://authority.example/mcp"))
        let params = AuthorityDetailView.neonApiKeyCourierParams(
        ...
        XCTAssertEqual(params.missingSecrets, ["NEON_API_KEY"])
