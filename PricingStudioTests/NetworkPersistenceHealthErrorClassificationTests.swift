...
/// The Network Persistence Health panel must NOT conflate a tool-discovery gap
/// (the Authority's wheel simply doesn't expose `network_persistence_health`) with a
...
/// `AuthorityDetailView.classifyPersistenceHealthLoad` is the seam that keeps them
/// apart, so it's pinned here.
final class NetworkPersistenceHealthErrorClassificationTests: XCTestCase {
    ...
    func testMissingToolClassifiesAsUnavailableNotHealthError() {
        ...
        guard case .unavailable(let msg) = AuthorityDetailView.classifyPersistenceHealthLoad(err) else {
    ...
    func testStructuredSoftErrorClassifiesAsHealthError() {
        ...
        XCTAssertEqual(
            AuthorityDetailView.classifyPersistenceHealthLoad(err),
    ...
    func testConnectionFailureClassifiesAsHealthError() {
        guard case .error = AuthorityDetailView.classifyPersistenceHealthLoad(
